...
sleep(3)
term.clear()
-- specify name of dump chest and pickup chest
...
while true do
  ...
  elseif key == "enter" then
    if beginsWith(word, "auto ") then
      local itemName = word:sub(6)
      clearUnderSearch()
      term.setCursorPos(1,2)
      term.write("Launching turtle miner for: "..itemName)
      local success, err = shell.run("miner", itemName)
      if success then
        notify("Miner finished successfully")
        silo.update_all_items()
      else
        notify("Miner error: "..tostring(err))
      end
      sleep(1)
      word = ""
